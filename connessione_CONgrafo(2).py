...
# Linea iniziale
line, = ax.plot([], [],'go:', label='Spessore vs Resistenza')
ax.set_xlabel("Resistenza (Ohm)")
ax.set_ylabel("Spessore (nm)")
ax.set_title("Spessore in funzione della Resistenza")
ax.legend()

# Imposta limiti fissi iniziali
ax.set_xlim(0, 10)  # Limiti per la resistenza
ax.set_ylim(0, 1000000)  # Limiti per lo spessore
...
libra.read(8) #leggiamo correta ricezione del messaggio di prendere le misure
...
while True:
    ...
    # Aggiungi i dati
    x_data.append(float(spessore))  #necessario creare altri array per costruire il grafo completo
    y_data.append(misura)
    
    # Aggiorna linea
    line.set_data(x_data, y_data)
    #line.set_data(array_x[1],array_y[2])
    # Redraw
    fig.canvas.draw()
    ...
    if keyboard.is_pressed("esc"):
        ...
        np.savetxt(fname= "libra_data.txt", X= libra_data,fmt='%s', delimiter = '\t', header = "Tempo\t Rate\t Spessore")
        libra.reset_input_buffer()
        libra.write(stop.encode('latin-1'))

        
        np.savetxt(fname= "multimeter_data.txt", X= multi_data,fmt='%s', delimiter = '\t', header = "Tempo\t Resistenza")
        multimeter.close()
        time.sleep(2)
        libra.reset_input_buffer()
